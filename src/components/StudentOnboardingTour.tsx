...
const STEPS: TourStep[] = [
  {
    key: "welcome",
    title: "Кабинет курсанта и правила",
    body: "Здесь — инструкторы, расписание, чат, билеты ПДД и история. Используйте достоверные данные, не передавайте вход третьим лицам; запись и талоны — по правилам школы и подсказкам в приложении. Далее — кратко по разделам меню. «Пропустить» — в любой момент.",
    navTarget: null,
  },
  {
    key: "home",
    title: "Главная",
    body: "Здесь отображаются профиль и статус аккаунта, баланс талонов и список ваших инструкторов (закреплённых за вами администратором). У каждого инструктора есть кнопка «Позвонить» — при указанном в профиле телефоне можно сразу начать звонок; кнопка «Чат» открывает переписку с этим инструктором. Ниже — недельное расписание: подтверждённые занятия, бронь свободных окон, отмена в допустимых случаях. Во время занятия — подтверждение старта вождения и таймер по подсказкам на экране.",
    navTarget: "home",
  },
  {
    key: "chat",
    title: "Чат",
    body: "Переписка с инструкторами и администратором. На значке может быть число непрочитанных сообщений.",
    navTarget: "chat",
  },
  {
    key: "tickets",
    title: "Билеты",
    body: "Тренировка по билетам ПДД (экзаменационные вопросы) для самостоятельной подготовки.",
    navTarget: "tickets",
  },
  {
    key: "history",
    title: "История",
    body: "Журнал прошедших и отменённых вождений, движение талонов по вашему балансу.",
    navTarget: "history",
  },
  {
    key: "settings",
    title: "Настройки",
    body: "Профиль, уведомления, приватность и оформление. Рекомендуем разрешить уведомления, чтобы не пропускать сообщения и напоминания о занятиях.",
    navTarget: "settings",
  },
];
