 ...
 * Учитываются все слоты кроме отменённых; все окна на дату, кроме `excludeWindowId` (текущее окно при брони).
 */
export function hasDriveTimeOverlapOnInstructorDay(
  ...
  for (const s of slots) {
    ...
    if (t == null) continue;
    let slotEndMs = t + DRIVE_SESSION_BLOCK_MS;
    if (s.status === "completed" && s.liveEndedAt != null) {
      // Early completion releases the remaining part of the 90-minute interval.
      slotEndMs = Math.max(t, Math.min(slotEndMs, s.liveEndedAt));
    }
